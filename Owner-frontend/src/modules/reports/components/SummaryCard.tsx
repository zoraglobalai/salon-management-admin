...
export function SummaryCard({
  ...
  return (
    <div
      className={cn(
        "group flex min-h-[120px] sm:min-h-[152px] flex-col rounded-[20px] border p-3.5 sm:p-5 transition-all duration-200",
        isDark
    ...
    >
      <div className="mb-2 sm:mb-3 flex items-start justify-between gap-2 sm:gap-3">
        <h3 className={`text-[12px] sm:text-[15px] font-medium leading-tight sm:leading-6 ${isDark ? "text-[#7A7572]" : "text-[#6B7280]"}`}>
          {title}
        </h3>
        {icon && (
          <div
      ...
      <div className="flex flex-1 flex-col justify-between gap-2 sm:gap-3">
        <div className={`text-[1.25rem] sm:text-2xl font-semibold tracking-[-0.03em] leading-none ${isDark ? "text-[#F0EBE3]" : "text-[#111827]"}`}>
          {value}
        </div>

        <div className="flex flex-wrap items-center gap-1 text-[10px] sm:text-sm">
          {trend === "up" && (
            <TrendingUp
